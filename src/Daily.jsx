...
const Daily = () => {
    const [dailyLevels, setDailyLevels] = useState([]);
        ...
        const fetchDaily = async () => {
            try {
                ...
                if (data && data.layouts) {
                    ...
                    setDailyLevels(levelsWithExpiry);
                }
            } catch (err) {
            ...
            {loading && dailyLevels.length === 0 ? (
                ...
                <div className="layout-browser-container glass" style={{ margin: '0 auto' }}>
                    {dailyLevels.map(level => (
                        <DailyLevelCard key={level.levelId} level={level} />
                    ))}
                </div>
            )}
        </div>
...
const DailyLevelCard = ({ level }) => {
    ...
    return (
        <div className="level-card glass daily-hue" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '1rem' }}>
