        ...
        const fetchMonthly = async () => {
            try {
                ...
                if (data && data.layouts) {
                    ...
                    setMonthlyLevels(levelsWithExpiry);
                }
            } catch (err) {
            ...
            {loading && monthlyLevels.length === 0 ? (
                ...
                <div className="layout-browser-container glass" style={{ margin: '0 auto' }}>
                    {monthlyLevels.map(level => (
                        <MonthlyLevelCard key={level.levelId} level={level} />
                    ))}
                </div>
            )}
        </div>
...
const MonthlyLevelCard = ({ level }) => {
    ...
    return (
        <div className="level-card glass monthly-hue" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '1rem' }}>
